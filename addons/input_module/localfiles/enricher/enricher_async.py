...
from enricher_db_async import AsyncEnricherDb
...
class MetadataEnricher:
    ...
    async def _enrich_track(self, track):
        ...
        if had_updates:
            logger.info(
                f"Updating track {track['id']} with new metadata: {updated_track}"
            )
...
async def _enricher_worker(config, db_manager: AsyncEnricherDb):
    ...
    logger.info("Metadata enricher task running")
        ...
        try:
            ...
            try:
                ...
                if command == "stop":
                    ...
                    await _enricher_instance.start()
                    ...
                    await _enricher_instance.process_changed_items(
