...
Recommends are left on, which is how `fpcalc` arrives for AcoustID fingerprinting — the build fails if it did not. The cost of that is a handful of packages that come in behind it and have no job here, so `EXCLUDED_PACKAGES` refuses them with an apt pin, and the build fails if one lands anyway. The expensive one is graphics: `fpcalc` links ffmpeg, ffmpeg's `libavutil` hard-depends `libva2` and `libvdpau1`, and those two Recommend `va-driver-all` and `vdpau-driver-all` — which is 250 MB of Mesa and a 118 MB `libLLVM`, to accelerate video on a headless machine that only ever decodes audio, on the CPU. The libraries themselves stay; only the drivers behind them are refused. Same story, smaller, for the cellular modem stack behind NetworkManager and X forwarding behind sshd. The pin is lifted before the image is sealed — it was about keeping the build lean, not about what you may install later.
