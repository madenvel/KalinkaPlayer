...
def _load_clap_onnx_module():
    """Import ClapOnnxModel WITHOUT triggering the kalinka_plugin_localfiles
    package __init__.

    clap_onnx.py only imports stdlib + numpy at module level, so it loads
    cleanly by file path. Going through the package (``from
    kalinka_plugin_localfiles.embedder.clap_onnx import ...``) runs the
    package __init__, which pulls in the SDK and can fail on unrelated WIP.
    """
    import importlib.util

    path = (Path(__file__).resolve().parents[4] / "packages"
            / "kalinka-plugin-localfiles" / "src" / "kalinka_plugin_localfiles"
            / "embedder" / "clap_onnx.py")
    spec = importlib.util.spec_from_file_location("clap_onnx_standalone", path)
    mod = importlib.util.module_from_spec(spec)
    sys.modules["clap_onnx_standalone"] = mod
...
def load_clap_encoder() -> Callable[[str], bytes]:
    ...
    Uses the PRODUCTION ONNX text encoder (ClapOnnxModel) by default. This is
    mandatory for valid numbers: the stored ``vec_tracks_clap*`` vectors were
    generated by the ONNX encoder, and laion_clap lives in a DIFFERENT
    embedding space — querying ONNX-audio vectors with laion_clap-text vectors
    is near-random (cross-space KNN, distances ~1.31). Set
    ``KALINKA_FORCE_LAION=1`` only for an explicit, clearly-flagged comparison.

    Model files are read from ``KALINKA_MODEL_DIR`` (default ``~/kalinka/models``).
    """
    import numpy as np
    ...
    if os.environ.get("KALINKA_FORCE_LAION") == "1":
        ...
        sys.stderr.write(
            "WARNING: KALINKA_FORCE_LAION=1 — encoding with laion_clap. Results "
            "will NOT match production (ONNX-audio vectors live in a different "
            "embedding space); use only for an explicit comparison.\n"
        )
        t0 = time.monotonic()
        model = laion_clap.CLAP_Module(enable_fusion=False)
        model.load_ckpt()
        sys.stderr.write(f"  laion_clap loaded in {time.monotonic() - t0:.1f}s\n")
        return lambda text: _normalise_blob(
            model.get_text_embedding([text], use_tensor=False)[0]
        )

    # Production-faithful default: ONNX text encoder.
    model_dir = os.environ.get(
        "KALINKA_MODEL_DIR", os.path.expanduser("~/kalinka/models")
    )
    sys.stderr.write(f"Loading CLAP via ONNX (production) from {model_dir} ...\n")
    ClapOnnxModel = _load_clap_onnx_module().ClapOnnxModel
    t0 = time.monotonic()
    model = ClapOnnxModel(model_dir=model_dir)
    model.load()
    if not model.is_loaded:
        raise RuntimeError(
            f"ClapOnnxModel failed to load from {model_dir}. Set KALINKA_MODEL_DIR "
            "to the dir holding clap_text_encoder.onnx / clap_tokenizer.json."
        )
    sys.stderr.write(f"  ONNX CLAP loaded in {time.monotonic() - t0:.1f}s\n")

    def encode(text: str) -> bytes:
        vec = model.get_text_embedding(text)  # single str -> 512-dim np.ndarray
        if vec is None:
            raise RuntimeError(f"ONNX text encoding returned None for {text!r}")
        return _normalise_blob(vec)
